import type { QuotaResetBank, QuotaWindow, SelfQuotaResponse } from "../types";
...
function isQuotaWindow(value: unknown): value is QuotaWindow {
  ...
  return (value.kind === "five_hour" || value.kind === "weekly")
...
function isResetBank(value: unknown): value is QuotaResetBank {
  if (!isRecord(value)) return false;
  return (value.status === "available" || value.status === "summary" || value.status === "unavailable")
...
function isSelfQuotaResponse(value: unknown): value is SelfQuotaResponse {
  ...
  return (quota.plan_type === undefined || typeof quota.plan_type === "string")
...
function errorPayload(value: unknown): { code?: string; message?: string } | undefined {
...
export async function fetchSelfQuota(
  ...
  if (!isSelfQuotaResponse(payload)) {
    throw new QuotaAPIError(response.status, "quota_response_invalid", "invalid quota response");
  }
  return payload;
}
