...
7. **可选 sidecar** — 过滤后的 `GET /v1/models`，客户只能看到自己的别名列表。
...
`sidecar.enabled: true` 时，插件在 `listen` 上监听并反代到 `upstream`。

1. `models_api_key` 填一把能在 CPA 上调 `GET /v1/models` 的上游 key。  
2. 终端用户只发 **插件** `cpa_…` key。  
3. 客户端 base URL 指向 sidecar（例如 `http://127.0.0.1:19090/v1`）。  

此时 `/v1/models` 只返回该 key 的别名；其它路径仍进 CPA（策略仍在上游执行一次）。
...
7. 客户：OpenAI 兼容 base URL = CPA（或 sidecar）；`Bearer cpa_…`；`model` = 别名。  
8. openai-compat 通道务必声明 models，否则会「无 auth」。  
9. （可选）开 sidecar，给客户过滤后的模型列表。
